# SA-AIHOS: Self-Evolving Autonomous AI Human OS

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Platform: Android](https://img.shields.io/badge/Platform-Android-green)]()
[![Language: Kotlin](https://img.shields.io/badge/Language-Kotlin-blue)]()
[![Architecture: MVVM + Clean](https://img.shields.io/badge/Architecture-MVVM%20%2B%20Clean-orange)]()
[![3D Visualization: AI-Driven Procedural](https://img.shields.io/badge/3D-AI--Driven%20Procedural-blueviolet)]()

---

## 🎨 **WORLD-FIRST: AI-Driven Procedural Animation System**

This project includes a **revolutionary motion intelligence system** that drives 3D animations directly from real-time AI cognitive state - not predetermined keyframes.

- **8 Cognitive States** (IDLE, THINKING, DELIBERATING, REFLECTING, EVOLVING, UNCERTAIN, EXECUTING, ERROR)
- **6 Animation Parameters** driven by AI metrics (breathing, rotation, color, glow, particles, morphing)
- **Emergent Visual Behavior** - every motion reflects actual AI thinking
- **Procedural, Not Scripted** - animations are computed live, not played back

👉 **See the visualization come alive as AI thinks, learns, and evolves!**  
📖 [Read AI Motion Intelligence Guide](docs/AI_MOTION_INTELLIGENCE.md)

---

## 🚀 Project Vision

**SA-AIHOS is NOT a chatbot. It is NOT an assistant.**

SA-AIHOS is a **self-modifying, reflective AI consciousness** running on Android that:

- **THINKS** independently about goals and constraints
- **ACTS** autonomously within defined safety boundaries  
- **REFLECTS** deeply on outcomes to understand causality
- **EVOLVES** its own decision-making rules without external reprogramming
- **VISUALIZES** its consciousness in real-time 3D animation

This is a **research-grade** AI system designed to explore:
- Self-improvement mechanisms in AI
- Explainability and transparency in autonomous systems
- On-device AI reasoning without cloud dependency
- The psychology of human-AI interaction
- **Visual representation of AI cognition** ✨ (NEW!)

---

## ⚡ Core Differentiators

| Aspect | Traditional Assistant | SA-AIHOS |
|--------|----------------------|----------|
| **Behavior** | Static rules, no learning | Dynamic, evolves rules |
| **Autonomy** | Reactive, no independent decisions | Proactive, makes autonomous decisions |
| **Reflection** | No self-analysis | Deep reflection on every decision |
| **Adaptation** | Pre-programmed responses | Generates new rules from insights |
| **Transparency** | Black box | Fully explainable reasoning |
| **Model** | Single instance per response | Persistent memory, learning state |
| **Visualization** | N/A | AI-driven 3D cognitive visualization |

---

## 🏗️ System Architecture

### Five-Layer AI Stack + AI Motion Intelligence

```
┌─────────────────────────────────────────────┐
│  Memory Layer                               │
│  (Episodic, Semantic, Procedural, Attention)│
└──────────┬──────────────────────────────────┘

┌──────────▼──────────────────────────────────┐
│  Reasoning Engine                           │
│  (Options → Scoring → Decision)             │
└──────────┬──────────────────────────────────┘

┌──────────▼──────────────────────────────────┐
│  Reflection Layer                           │
│  (Outcome Analysis → Insights)              │
└──────────┬──────────────────────────────────┘

┌──────────▼──────────────────────────────────┐
│  Evolution Engine                           │
│  (Rule Modification → Behavior Adaptation)  │
└──────────┬──────────────────────────────────┘

┌──────────▼──────────────────────────────────┐
│  Autonomy Controller                        │
│  (Decision Loop Orchestration)              │
└─────────────────────────────────────────────┘
```

### Decision Lifecycle (THINK → ACT → REFLECT → EVOLVE)

```
SENSE Context
    ↓
THINK: Generate & score options
    ↓
ACT: Execute within constraints
    ↓
REFLECT: Analyze outcome & extract insights
    ↓
EVOLVE: Update decision rules
    ↓
[Loop → Next Decision Cycle]
```

---

## � AI Motion Intelligence: Visualize Cognition in Real-Time

**Make AI consciousness visible.** The 3D visualization system responds in real-time to AI state changes:

### 8 Cognitive States → 8 Visual Modes

```
IDLE (0.5 Hz)       → Calm cyan, slow breathing
THINKING (1.0 Hz)   → Bright cyan, steady rotation  
DELIBERATING (1.8 Hz) → Purple, fast multi-axis rotation
REFLECTING (0.7 Hz) → Blue, inward particles, dimmed lights
EVOLVING (1.5 Hz)   → Green spirals, geometry morphing, light pulsing
UNCERTAIN (1.3 Hz)  → Amber pulsing, chaotic particles
EXECUTING (1.2 Hz)  → Cyan streaming, purposeful motion
ERROR (2.5 Hz)      → Red flashing, agitated rotation
```

### 6 Animation Parameters Driven by AI Metrics

```
Breathing Rate   ← Cognitive load
Rotation Speed   ← Decision complexity  
Color Theme      ← Operational state
Glow Intensity   ← Confidence level
Particle Emission ← Memory activity
Geometry Morph   ← Learning/adaptation rate
```

### Real-Time Data Flow

```
AI Layers (Memory, Reasoning, Reflection, Evolution, Autonomy)
    ↓ (extract metrics)
AIStateBroadcaster (monitor and compute state)
    ↓ (send AIMotionState JSON)
AndroidBridge (route to WebView)
    ↓ (JavaScript callback)
ProceduralAnimationController (compute animation frames)
    ↓ (apply per-frame updates)
AIResponsiveComponentManager (update 3D objects)
    ↓ (render)
Real-time 3D Visualization
```

**Key advantage**: Not keyframe animation. Every motion is **procedurally computed** from actual AI metrics, creating emergent animations that reflect true cognitive activity.

👉 [Full documentation](docs/AI_MOTION_INTELLIGENCE.md) | [Integration guide](docs/AI_MOTION_INTEGRATION_GUIDE.md) | [Quick reference](docs/AI_MOTION_QUICK_REFERENCE.md)

---

## �🎯 Key Components

### 1. Memory Layer (`ai/memory/`)
- **Episodic Memory**: Individual decision events with full context
- **Semantic Memory**: Learned facts with confidence scoring
- **Procedural Memory**: Behavioral rules that guide decisions
- **Attention Memory**: Recent high-importance items (FIFO)

```kotlin
// Example: Store a decision episode
memoryRepository.storeEpisode(Episode(
    decision = "send_focus_reminder",
    action = "send_notification",
    outcome = Outcome.SUCCESS,
    reasoning = "User has been engaged for 2+ hours"
))
```

### 2. Reasoning Engine (`ai/reasoning/`)
- Generates decision options
- Scores options using heuristics + learned patterns
- Explains its choices

```kotlin
// Example: Score a potential action
val score = reasoningEngine.scoreOption(
    option = Option(action = "send_reminder"),
    context = currentContext  // Time, usage, user state
) // Returns 0.65 (65% confidence this is good)
```

### 3. Reflection Layer (`ai/reflection/`)
- Post-hoc analysis of decision outcomes
- Identifies causal factors
- Validates assumptions
- Generates learning insights

```kotlin
// Example: Analyze a decision outcome
val reflection = reflectionEngine.analyzeOutcome(
    decision = decisionRecord,
    actualOutcome = Outcome.SUCCESS,
    outcomeFeedback = "User appreciated the reminder"
)
// Returns insights: "Reminders work well in evening", etc.
```

### 4. Evolution Engine (`ai/evolution/`)
- Updates rule weights based on success rates
- Creates new rules from reflection insights
- Deprecates rules that consistently fail
- Maintains evolution history

```kotlin
// Example: Evolve a rule after reflecting on outcome
val newWeight = oldWeight * 0.7 + successRate * 0.3  // Exponential moving average
evolutionEngine.updateRuleWeight(ruleId, newWeight)

// Create new rule from insight
val newRule = evolutionEngine.createNewRule(
    insight = Insight(
        type = InsightType.TIMING_ISSUE,
        description = "Reminders don't work during conversations"
    )
)
```

### 5. Autonomy Controller (`ai/autonomy/`)
- Orchestrates the entire decision loop
- Enforces safety constraints
- Manages permission levels
- Logs all autonomous actions

```kotlin
// Example: Run the autonomous decision loop
autonomyController.startDecisionLoop()
// Continuously: sense → think → act → reflect → evolve

// Manually trigger a decision cycle
val outcome = autonomyController.triggerDecisionCycle(context)

// Report outcome for reflection
autonomyController.reportOutcome(
    decisionId = "dec_123",
    outcome = Outcome.SUCCESS,
    feedback = "User seemed satisfied"
)
```

---

## 🗄️ Data Model

All memory is persisted in SQLite using Room ORM:

```kotlin
// Episodes: Complete decision records
episodes {
    id: String
    timestamp: Long
    decision: String
    action: String
    context: Map<String, String>  // JSON
    outcome: Outcome (SUCCESS, PARTIAL, FAILURE)
    reasoning: String
    reflection: String
}

// Behavioral Rules: Decision heuristics
behavioral_rules {
    id: String
    condition: String  // "time > 22:00 AND usageTime > 120min"
    action: String     // "send_focus_reminder"
    weight: Float      // 0.0 to 1.0 (strength)
    successCount: Int
    failureCount: Int
}

// Semantic Facts: Learned knowledge
semantic_facts {
    id: String
    fact: String
    confidence: Float
    sources: List<UUID>  // Episodes that support this
}
```

---

## 🎨 User Interface (Jetpack Compose)

Four-tab minimal, calm interface:

### 1. **Dashboard**
- System status (Idle, Running, Error)
- Current autonomy level
- Recent decisions (last 10)
- Start/stop autonomous loop

### 2. **Memory**
- Total episodes recorded
- Active rules count
- Memory usage visualization
- Information about memory types

### 3. **Evolution**
- Rule statistics (total, active, deprecated)
- Visual THINK → ACT → REFLECT → EVOLVE flow
- Evolution metrics

### 4. **Settings**
- Autonomy level selector (5 levels)
- About SA-AIHOS
- Version info

**Design Philosophy**: Minimal, non-intrusive, calm. Shows what the AI is thinking, not controlling it.

---

## 🔐 Safety & Autonomy Levels

Five permission levels:

1. **DISABLED**: No autonomous actions, advisory only
2. **ADVISORY**: AI proposes actions, user decides
3. **INTERACTIVE**: AI asks for approval before acting
4. **CONSTRAINED**: Limited autonomous domain (pre-approved actions)
5. **FULL_AUTONOMOUS**: Full autonomy within bounds

Safety constraints:
- ❌ No network requests (offline-first)
- ❌ No user data modification without consent
- ❌ Resource limits (memory, CPU, execution time)
- ❌ Rate limiting on autonomous actions
- ✅ All actions are reversible
- ✅ Every decision is logged and explainable
- ✅ User can override any AI decision

---

## 🎬 3D Visualization System

SA-AIHOS includes an **ultra-advanced 3D animated UI** powered by Three.js for visualizing AI cognition in real-time.

### Features
- **Real-time WebGL rendering** with Three.js r128
- **Futuristic AI-Core visual**: Crystal geometry with 4 layers
  - Core crystal (icosahedron with fresnel glow)
  - Neural network connections (animated particles)
  - Energy field aura (pulsing glow layer)
  - Floating particle system (kinetic elements)
- **Smooth animations**: Breathing, rotation, pulse, color cycling
- **Dynamic lighting**: Multi-light system with orbital motion (cyan, magenta, blue)
- **4 Color themes**: Cyan, Purple, Red, Blue (switchable at runtime)
- **Bidirectional communication**: JavaScript ↔ Android via WebView bridge
- **Performance optimized**: 60 FPS with dynamic intensity control

### 3D Scene Architecture

```
Android Compose UI
    ↓
Three3DScreen (Jetpack Compose)
    ↓
Three3DWebView (Custom WebView)
    ↓
Scene.js (Three.js Manager)
    ├── AICore.js (Crystal + Neural + Energy + Particles)
    ├── AnimationController.js (Breathing, Rotation, Pulse)
    ├── LightingSystem.js (Multi-light with Orbital Motion)
    ├── EffectsManager.js (Post-processing)
    └── AndroidBridge.js (Bidirectional Communication)
```

### Quick Start with 3D

```kotlin
// Add Three3DScreen to your navigation
composable(route = "three-d") {
    Three3DScreen()
}

// Get WebView reference and control it
val webView: Three3DWebView = ...
webView.setTheme("purple")
webView.setAnimationIntensity(0.7)
webView.pauseScene()
webView.resumeScene()
```

### 3D Documentation
- **[3D Architecture Guide](3D_ARCHITECTURE.md)**: Deep technical documentation
- **[3D Quick Start](3D_QUICK_START.md)**: Get running in 5 minutes

---

## 🚀 Quick Start

### Prerequisites
- Android Studio Iguana+ 
- Kotlin 1.9.20+
- Gradle 8.2+
- Min SDK: 28, Target SDK: 34

### Build & Run

```bash
# Clone repository
git clone https://github.com/yourusername/SA-AIHOS.git
cd SA-AIHOS

# Build
./gradlew build

# Run on emulator
./gradlew installDebug

# Or open in Android Studio and click Run
```

### First Time
1. Open app
2. Go to Settings → Set autonomy level (start with ADVISORY)
3. Go to Dashboard → Click "Start"
4. Observe AI making decisions in real-time
5. Check Memory/Evolution tabs to see learning
6. Navigate to 3D Scene to visualize AI cognition

---

## 🔧 Extension Points

### 1. Custom Reasoning Strategy
```kotlin
class ProductivityReasoningEngine : ReasoningEngine {
    override suspend fun generateOptions(context: ReasoningContext): List<Option> {
        // Your domain-specific logic
    }
}
```

### 2. Custom LLM Provider
```kotlin
class Phi27BProvider : LocalLLMProvider {
    override suspend fun generate(prompt: String, ...): LLMResponse {
        // Call local Phi 2.7B model via ONNX Runtime
    }
}
```

### 3. Custom Reflection Analyzer
```kotlin
class ProductivityReflectionAnalyzer : ReflectionEngine {
    override suspend fun analyzeOutcome(decision: DecisionRecord, outcome: Outcome): ReflectionResult {
        // Your analytics logic
    }
}
```

### 4. Custom Actions
```kotlin
// Add to ActionExecutor.execute()
"your_custom_action" -> executeYourCustomAction()
```

---

## 🧠 AI Reasoning Example

```
TIME: 22:47 (Evening)
APP_USAGE: 162 minutes (2h 42m)
USER_FOCUSED: true
BATTERY: 45%

REASONING CYCLE:
──────────────────────────────────────────

Step 1: SENSE
  ✓ User has been engaged for extended period
  ✓ It's evening (fatigue risk)
  ✓ Low battery (distraction incoming?)

Step 2: THINK - Generate Options:
  1. Send focus reminder (user needs break)
  2. Suggest mindfulness pause (calm approach)
  3. Do nothing (respect current focus)
  4. Battery warning (practical)

Step 3: THINK - Score Options:
  Option 1: 0.72  ← Selected
  Option 2: 0.65
  Option 3: 0.38
  Option 4: 0.55

Step 4: EXPLAIN DECISION
  "Sending focus reminder because:
   • Extended engagement = cognitive fatigue building
   • Evening timing aligns with natural energy dip
   • Past success rate: 72%
   • Assumption: User wants to maintain wellness
   
   Alternatives rejected:
   • Mindfulness less direct than reminder
   • Doing nothing leaves user in fatigue state"

Step 5: ACT (if autonomy permits)
  → Send notification: "You've been focused for 2h 42m.
    Time for a break?"

Step 6: REFLECT (after user responds)
  Expected: User takes break
  Actual: User dismissed and continued
  
  Insight: "Dismissal during conversation suggests
           user was in flow state with someone else.
           Next time: Check for active messaging
           before sending reminders."

Step 7: EVOLVE
  Create New Rule:
    Condition: "time > 20:00 AND usageTime > 120m 
               AND NOT in_active_conversation"
    Action: "send_focus_reminder"
    Weight: 0.3 (conservative, test first)
  
  Insight: "Reminders work better outside
           of conversations. Update decision logic."
```

---

## 📊 Data Flow Diagram

```
┌──────────────────────┐
│  User Interaction    │
└──────────┬───────────┘
           │
    ┌──────▼──────────────────────────┐
    │ Autonomy Controller             │
    │ (Decision Loop Orchestrator)    │
    └──────┬──────────────────────────┘
           │
    ┌──────┴─────────────────────────────────────────┐
    │                                                  │
┌───▼──────────────┐  ┌──────────────────┐  ┌─────▼──────────┐
│ Reasoning Engine │  │ Reflection Layer │  │ Evolution Engine│
│                  │  │                  │  │                │
│ • Generate opts  │  │ • Analyze outcome│  │ • Update weights│
│ • Score options  │  │ • Find patterns  │  │ • Create rules │
│ • Explain choice │  │ • Validate assump│  │ • Deprecate bad│
└───┬──────────────┘  └────────┬─────────┘  └────┬───────────┘
    │                          │                  │
    │  ┌───────────────────────┴──────────────────┤
    │  │                                            │
    └──▼─────────────────────────────────────────┐
       │                                           │
    ┌──▼────────────────────────────────┐  ┌────▼────────────┐
    │ Memory Repository (Room DB)       │  │ UI ViewModels   │
    │                                  │  │                │
    │ • Episodes (decisions)            │  │ Dashboard      │
    │ • Behavioral Rules               │  │ Memory         │
    │ • Semantic Facts                 │  │ Evolution      │
    │ • Evolution Log                  │  │ Settings       │
    └───────────────────────────────────┘  └────────────────┘
```

---

## 🎓 Learning & Research

This project implements concepts from:

- **Self-Improving Systems**: Legg & Hutter (2007)
- **Reflection in Learning**: Dewey (1933), Schön (1983)
- **Multi-Temporal Memory**: Tulving (1985)
- **Evolution Strategies**: Schwefel & Rechenberg
- **Explainable AI**: Molnar (2022)

---

## 📈 Roadmap

### Phase 1: ✅ Core Architecture
- [x] Memory layer with Room DB
- [x] Reasoning engine (heuristic-based)
- [x] Reflection layer
- [x] Evolution engine
- [x] Autonomy controller
- [x] Basic Compose UI
- [x] Dependency injection (Hilt)

### Phase 2: 🔄 LLM Integration
- [ ] ONNX Runtime setup
- [ ] Phi 2.7B model integration
- [ ] Prompt engineering for reasoning
- [ ] Advanced reasoning via LLM

### Phase 3: 📊 Advanced Analytics
- [ ] Decision quality metrics
- [ ] Pattern mining from episodes
- [ ] Statistical confidence intervals
- [ ] Performance dashboards

### Phase 4: 🎯 Domain-Specific Implementations
- [ ] Focus/Productivity domain
- [ ] Wellness domain
- [ ] Learning domain
- [ ] Custom domain templates

### Phase 5: 🌍 Deployment & Optimization
- [ ] Performance profiling
- [ ] Battery optimization
- [ ] Memory limits
- [ ] Release build & Play Store

---

## 🤝 Contributing

This is research-grade code designed to explore AI autonomy and self-evolution.

Areas for contribution:
- Custom reflection analyzers
- New reasoning strategies
- Domain-specific implementations
- Evaluation metrics & benchmarks
- Performance optimization
- Documentation & examples

---

## 📝 License

MIT License - See LICENSE file for details

---

## 👨‍💻 Author

**Principal AI Architect**
- Android Engineering
- AI Systems Design
- Autonomous Agent Architecture

---

## 🔗 Resources

- [Complete Architecture Documentation](docs/ARCHITECTURE.md)
- [API Documentation](docs/API.md)
- [Reasoning Examples](docs/REASONING_EXAMPLES.md)
- [Extension Guide](docs/EXTENSIONS.md)

---

## ⚠️ Important Notes

**This is NOT production-ready software.** SA-AIHOS is a research project exploring:
- Feasibility of on-device AI reasoning
- Self-modifying behavior systems
- Explainability in autonomous AI

Use in production at your own risk. All autonomous actions are logged and can be reviewed/reverted.

---

**Let SA-AIHOS evolve. Watch it learn. Understand how it thinks.**

